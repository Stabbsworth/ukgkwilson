...
Ver 1.4 - Removed the shiori3.dic from the YAYA version since it's not necessary, added a fix from ponapalt to anchor.dic that should let envelopes display properly when using script input, fixed the description for the secondary OnFirstBoot dialogue. Figured out what commu.dic does and rewrote the file accordingly.
